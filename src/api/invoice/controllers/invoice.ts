...
export default factories.createCoreController('api::invoice.invoice', ({ strapi }) => ({
  async generate(ctx) {
    ...
    try {
      ...
      ctx.status = 500;
      ctx.body = { error: 'Failed to generate invoice', message: err?.message, stack: err?.stack };
    }
  },
  ...
  async download(ctx) {
    ...
    try {
      ...
      if (!record) return ctx.notFound('Invoice not found');
      ...
      ctx.status = 500;
      ctx.body = { error: 'Failed to download invoice', message: err?.message, stack: err?.stack };
    }
  }
}));
