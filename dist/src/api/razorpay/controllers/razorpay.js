...
exports.default = {
    async createOrder(ctx) {
        try {
            ...
            const order = await razorpay_1.default.createOrder(amount, currency);
    ...
    async verifyPayment(ctx) {
        try {
            const { razorpay_order_id, razorpay_payment_id, razorpay_signature, customer_email, customer_name, customer_phone, tenantId, } = ctx.request.body;
            if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
                return ctx.badRequest('Missing required fields');
            }
            const isValid = razorpay_1.default.verifyPayment(razorpay_order_id, razorpay_payment_id, razorpay_signature);
